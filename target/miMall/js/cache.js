...
layui.use(['form','jquery',"layer"],function() {
    ...
    function showNotice(){
        layer.open({
            ...
            btn: ['火速围观'],
            moveType: 1,
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">欢迎来到小米商城后台管理</p><p>本管理系统基于layui制作，用于处理商品、用户、订单相关管理，接下来请进行您的操作。</p></div>',
    ...
    $("body").on("click","#unlock",function(){
        if($(this).siblings(".admin-header-lock-input").val() == ''){
        	...
        	var input = $(this).siblings(".admin-header-lock-input").val();
        	$.ajax({
        		url:"/miMall/getPassword",
        		dataType:"json",
        		type:"POST",
